#include "imu.h"
#include <stdio.h>
#include "Serial.h"

#define UP_BYTE1 		0x55        //帧头1
#define UP_BYTE2 		0x55        //帧头2
#define UP_ATTITUDE     0x53		//姿态角ID

#define RECEIVE_OK  1       //成功解析标志

/* 模块上传有效数据长度 */
#define ATKP_MAX_DATA_SIZE 8

/* 模块上传姿态角数据 */
ATTITUDE_t  attitude;		  

/* 串口接收数据包 */
IMU901_t    rxPacket_imu901;

/* 串口接收数据包流程 */
static enum
{
    waitForStartByte1,
//    waitForStartByte2,
    waitForMsgID,
//    waitForDataLength,
    waitForData,
    waitForChksum1,
} rxState = waitForStartByte1;

/* 中断接收缓冲区 */
uint8_t IMU901_Rx_buffer;

/* 是否接收陀螺仪数据 */
bool if_imu_open = false;

/**********************************************************************
 * @Name    receive_imu901_Init
 * @declaration : 初始化串口5 
 * @param   attitude 姿态角结构体
 * @retval   : 无
 * @author  hoson_stars
 ***********************************************************************/
void receive_imu901_Init(ATTITUDE_t *attitude){
	attitude->huart=&huart5;
    HAL_UART_Receive_IT(attitude->huart,&IMU901_Rx_buffer, 1);
}

/**********************************************************************
 * @Name    receive_imu901_IRQ
 * @declaration : 串口5中断接收一个字节
 * @param   attitude 姿态角结构体
 * @retval   : 无
 * @author  hoson_stars
 ***********************************************************************/
void receive_imu901_IRQ(ATTITUDE_t *attitude)
{
     if(RECEIVE_OK==unpacked_imu901(IMU901_Rx_buffer)) {
        Get_imu901(&rxPacket_imu901);
     }
     HAL_UART_Receive_IT(attitude->huart,&IMU901_Rx_buffer, 1); 
}

/**********************************************************************
 * @Name    unpacked_imu901
 * @declaration : 解析imu901传回单个字节信息
 * @param   ch 接收到的字节 
 * @retval   : 0/1 解析状态
 * @author  hoson_stars
 ***********************************************************************/
uint8_t unpacked_imu901(uint8_t ch)
{
    static uint8_t cksum = 0, dataIndex = 0;

    switch (rxState)
    {
        case waitForStartByte1:
            if (ch == UP_BYTE1)
            {
//                rxState = waitForStartByte2;
				rxState = waitForMsgID;
                rxPacket_imu901.startByte1 = ch;
            }

            cksum = ch;
            break;

//        case waitForStartByte2:
//            if (ch == UP_BYTE2)
//            {
//                rxState = waitForMsgID;
//                rxPacket_imu901.startByte2 = ch;
//            }
//            else
//            {
//                rxState = waitForStartByte1;
//            }

//            cksum += ch;
//            break;

        case waitForMsgID:
			if(ch == UP_ATTITUDE)
			{
				rxPacket_imu901.msgID = ch;
				rxState = waitForData;
				cksum += ch;
			}
			else
			{
				rxState = waitForStartByte1;
			}
            break;

//        case waitForDataLength:
//            if (ch <= ATKP_MAX_DATA_SIZE)
//            {
//                rxPacket_imu901.dataLen = ch;
//                dataIndex = 0;
//                rxState = (ch > 0) ? waitForData : waitForChksum1;	/*ch=0,数据长度为0，校验1*/
//                cksum += ch;
//            }
//            else
//            {
//                rxState = waitForStartByte1;
//            }

//            break;

        case waitForData:
            rxPacket_imu901.data[dataIndex] = ch;
            dataIndex++;
            cksum += ch;

            if (dataIndex == ATKP_MAX_DATA_SIZE)
            {
                rxState = waitForChksum1;
            }

            break;

        case waitForChksum1:
            if (cksum == ch)	/*!< 校准正确返回1  cksum == ch */
            {
                rxPacket_imu901.checkSum = cksum;
				rxState = waitForStartByte1;
                return 1;
            }
            else	/*!< 校验错误 */
            {
                rxState = waitForStartByte1;
            }
			cksum = 0;
			dataIndex = 0;
            rxState = waitForStartByte1;
            break;

        default:
			cksum = 0;
			dataIndex = 0;
            rxState = waitForStartByte1;
            break;
    }
    return 0;
}


/**********************************************************************
 * @Name    Get_imu901
 * @declaration : 获取imu传回数据帧中的姿态角信息
 * @param   packet 通讯数据结构 
 * @retval   : 无
 * @author  hoson_stars
 ***********************************************************************/
void Get_imu901(IMU901_t *packet)
{
    /* 姿态角 */
    if (packet->msgID == UP_ATTITUDE)
    {
        int16_t data = (int16_t) (short)(((short)packet->data[1] << 8) | packet->data[0]);
        attitude.roll = (float) data / 32768 * 180;

        data = (int16_t) (short)(((short)packet->data[3] << 8) | packet->data[2]);
        attitude.pitch = (float) data / 32768 * 180;
        
		data = (int16_t) (short)(((short)packet->data[5] << 8) | packet->data[4]);
        attitude.yaw = (float) data / 32768 * 180;
		printf("%f,%f,%f\n",attitude.roll,attitude.pitch,attitude.yaw);
    }
}



